...
#ifndef LEETCODE_410_H
#define LEETCODE_410_H
...
public:
    int splitArray(std::vector<int>& nums, int m) {
        int n = nums.size();
        std::vector<std::vector<long long>> f(n + 1, std::vector<long long>(m + 1, LLONG_MAX));
        std::vector<long long> sub(n + 1, 0);
        for (int i = 0; i < n; i++) {
    ...
    void test() {
        ...
        for (int i = 0; i < inputs.size(); ++i) {
            int ans = splitArray(inputs[i].first, inputs[i].second);
            std::cout << "ans: " << ans << std::endl << (ans == outputs[i]) << std::endl;
        }
    }
};

#endif //LEETCODE_410_H
